...
public class UserController {
    ...
    */
    ...
    @ResponseBody
    @GetMapping(path = RequestEndPoints.GET_USER_POSTS)
    public ResponseDTO getUserPosts(@RequestParam("userId") Long userId) {
        ResponseDTO<UserDetailPageDTO> responseDTO = userService.getUserPosts(userId);
        if (Objects.nonNull(responseDTO)) {
            logger.info(LoggerUtil.setLoggerInfoWithoutUser(this.getClass().toString(), responseDTO.getMessage()));
        }
        return responseDTO;
